...
int main(void)
{
	...
	// T?m bufferen for opstarts-st?j fra ESP'en
	...
	if (hentSensorStatus())
	{
		...
		while(1); // L?s systemet her ved kritisk fejl
	}
	...
	_delay_ms(1000);
	visBesked(); // Skriver "Batteri:" og "Gen.eff:" p? LCD
	...
	TIMSK4 = 0b00000001;
	ADCSRA = 0b11000111; // T?nd ADC med prescaler 128
	...
	while (1)
	{
		...
		// B. Opdater LCD og h?ndt?r tidsbaseret logik (Hver 0.5 sek)
		if (data_klar) {
			...
			// --- NY LOGIK: T?ller til fremmedlegeme-tjek ---
			if (varme_aktiv) {
				varme_timer++;
			}
		...
		// C. Motor-styring (K?rer nu flydende uden pauser!)
		...
		if ((float)gen_eff < threshold) {
			...
			if (temp > 22.0) {
				// ?jeblikkelig fejl: Varmt + ingen str?m
			...
			// Tjek om der er g?et 10 sekunder (20 ticks a 0.5 sek)
			if (varme_aktiv && varme_timer >= 20) {
				if ((float)gen_eff < threshold) {
					PORTB |= (1 << PB6);  // Stadig ingen str?m -> Fremmedlegeme!
					PORTB &= ~(1 << PB5); // Sluk varme
				}
				// Vi lader varme_aktiv v?re 1, s? den ikke "genstarter" opvarmningen hver gang
			}
		}
		else {
